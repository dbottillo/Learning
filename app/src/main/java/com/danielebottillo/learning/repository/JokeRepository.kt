...
import com.danielebottillo.learning.CoolSubject
...
class JokeRepository private constructor(val restClient: RestClient = RestClient()) {
    ...
    var mObservable: Subject<Joke> = CoolSubject.create<Joke>()
    ...
    fun getJoke() {
        ...
        restClient.random().map { Joke(it.value.text) }.subscribeOn(Schedulers.io())
            ...
            mObservable.onNext(it)
        }, {
            mObservable.onError(it)
        })
    }
    ...
    fun generateError() {
        mObservable.onError(Throwable("damn"))
    }
}
